<?php
namespace Kernel\Client\WebSocket;
...
class WebSocketClient
{
    ...
    {
        $this->client = new SwooleClient(SWOOLE_SOCK_TCP, SWOOLE_SOCK_ASYNC);
    ...
    public function onReceive($cli, $data)
    {
        ...
        } elseif ($this->state == static::HANDSHAKED) {
            $this->buffer .= $data;
        }
    ...
    public function on($event, $callback)
    {
        ...
        } elseif (strcasecmp($event, "message") === 0) {
            $this->messageCb = $callback;
        } elseif (strcasecmp($event, "close") === 0) {
    ...
    public function send($data, $type = 'text')
    {
        ...
        $data = SwooleWebsocketServer::pack($data, $_type);
