...
class RedisAsynPool extends AsynPool
{
    ...
    {
        if (Server::getInstance()->processType == Marco::PROCESS_TASKER) {//如果是task进程自动转换为同步模式
            return $this->getSync()->$name(...$arg);
        } else {
            // return $context->getObjectPool()->get(Redis::class, [$this, $name, $arg]);
            return $context->get('server')->container->make('Kernel\Coroutine\Redis', [$this, $name, $arg]);
            // return $context->getObjectPool()->get(Redis::class, [$this, $name, $arg]);
        }
    }
    ...
    public function getSync()
    {
        ...
        $conf = $this->config[$this->active];
        $addr = $conf['ip'] . ':' . $conf['port'] . ' ';
        $password = $conf['password'] ?? false;
        $select = $conf['select'] ?? false;
        ...
        try {
            ...
            if ($select) {//存在验证
                ...
                if (!$this->redisClient->select($select)) {
                    throw new Exception($this->redisClient->getLastError());
                }
            }
        } catch (\Exception $e) {
    ...
    public function reconnect($client = null)
    {
        ...
        $client->connect($this->connect[0], $this->connect[1], function ($client, $result) {
            ...
            if (!$result) {
                Logger::getInstance()->log('error', 'RedisAsynPool', [
                    'data' => $client->errMsg . " with Redis {$this->connect[0]}:{$this->connect[1]}"
                ]);
                return false;
            }
